...
- `D3_FIELD_LATEST` 拦截表「是否最新记录」单选字段的 field id（建好字段后填入）
...
#### 拦截表「是否最新记录」字段与 SKU 唯一索引（重要）

拦截表以 **SKU 编码为唯一业务索引**，商品名仅展示、不参与逻辑（客服/运营把品名填错也不会重复建档）。
实现方式是单选字段「是否最新记录」（选项 `是` / `否`）：

- 一个 SKU 同一时刻只有一行 `是`（活跃行），状态在 待拦截→已拦截→待恢复 间流转。
- 同一 SKU 再次提交：**不新建行、不耗批次**，只在「提交人」字段去重追加名字；恢复时会 @ 全部提交人和恢复发起人。
- 活跃行处于「待恢复」时又被提交：自动**撤销恢复**、回到已拦截（此时 D3 规则尚未删除）。
- D3 恢复成功 → 状态改已恢复、是否最新改 `否`（成为历史行，可在默认视图筛掉）。
- 恢复连续失败 3 次熔断：状态保持待恢复、是否最新置 `否`、写入错误信息并 @ 相关人，转人工处理。

**初始化步骤**：
1. 在拦截表手动新建单选字段「是否最新记录」，选项 `是`、`否`，把其 field id 填入 `config.env` 的 `D3_FIELD_LATEST`。
2. 对存量记录回填该字段：`python3 scripts/migrate-latest-flag.py dry-run` 看清单，确认后 `apply`。
3. 建议在多维表配两个视图：默认视图筛选 是否最新记录=`是`；「需人工处理」视图筛选 是否最新记录=`否` 且 状态=`待恢复`。

### 4.1 需要提前建好的钉钉多维表

同一个 Base 下需要 4 张表（字段见 `docs/SKILL.original.md` 与各脚本顶部的 `F_*` 常量）：
1. **自动客审拦截表**：SKU、品名、类型(单品/套装)、状态(待拦截/已拦截/待恢复/已恢复)、**是否最新记录(单选 是/否)**、批次、备注、提交人、各时间戳、错误。
